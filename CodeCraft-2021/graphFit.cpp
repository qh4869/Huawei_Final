...
void subGraphFit(cServer &server, cVM &VM, const cRequests &request, int begin, int end, int whichDay) {
	...
	for (int indexTerm = begin; indexTerm < end + 1; indexTerm++) {   // indexTerm 表示该Term在每天的实际顺序
		...
		if (iTerm < maxLoc - 1) {  
			...
			if (!requestTerm.type) {   // 表示要删除虚拟机
				hasDeploy[iTerm] = true;    // 删除的虚拟机不需要重新部署
	...
	server.price += minCost.back();    // 服务器一直开着的价格，如果alpha值为特定值时
...
void deleteVM(cServer &server, cVM &VM, sRequestItem &requestTerm, unordered_map<string, sVmItem> &workVm, vector<pair<string, int>> &restID) {
	...
	sVmItem requestVm = VM.info[tempVm.vmName];    // 提取要删除的虚拟机
	...
	if (search != VM.workingVmSet.end()) {      // 表示该虚拟机在前几天的服务器里
		...
		//////////////////////////////////////////////////////////////////////////
		server.updateRank(serID, false);   // 表示从里面拿走VM
		/////////////////////////////////////////////////////////////////////////
	}
...
void addVM(cServer &server, cVM &VM, sRequestItem &requestTerm, vector<bool> &hasDeploy, vector<vector<sServerItem>> &transfer,
	...
	if (myServer.energyCost < 0) {    // 表示能加入到已购买的服务器
		...
		if (requestVm.nodeStatus) {   // true 表示双节点
			...
			hasDeploy[iTerm] = true;
			//int flag = VM.deploy(server, whichDay, requestTerm.vmID, requestTerm.vmName, myServer.buyID, true);  // a 节点
			int flag = VM.deploy(server, whichDay, requestTerm.vmID, requestTerm.vmName, myServer.buyID, true, indexTerm);  // a 节点
			if (flag == 2) {
				//VM.deploy(server, whichDay, requestTerm.vmID, requestTerm.vmName, myServer.buyID, false);  // b 节点
				VM.deploy(server, whichDay, requestTerm.vmID, requestTerm.vmName, myServer.buyID, false, indexTerm);  // b 节点
			}
		}
		///////////////////////////////////////////////////////////////////////////
		server.updateRank(myServer.buyID, true);   // 往里加
		///////////////////////////////////////////////////////////////////////////
	}
	else {    // 表示加入到这一轮新购买的服务器

		hasDeploy[iTerm] = false;    // 属于新购买的服务器
...
void buyServer(cServer &server, cVM &VM, const cRequests &request, vector<pair<string, int>> &restID,
	...
	vector<sServerItem> buyServer;   // 表示要买的服务器
	int first = 0;
	int second = 0;
