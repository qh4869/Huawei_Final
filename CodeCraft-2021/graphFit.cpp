...
// 每天执行graphFit
void graphFitEachDay(cServer &server, cVM &VM, cRequests &request, int whichDay) {
	vector<double> args(5);
	args[2] = 0.35; 
	args[3] = 1.0;
	args[4] = 0;
	int vmTotalNum = VM.workingVmSet.size();
	...
	int cntMig = 0, migrateNum = vmTotalNum * 30 / 1000;
	if (whichDay > 0) {
		migrateVM_4(server, VM, whichDay, dayWorkingVM, vmTotalNum, delSerSet, args, cntMig, migrateNum,
			3);
	}
...
void deleteVM(cServer &server, cVM &VM, sRequestItem &requestTerm, unordered_map<string, sVmItem> &workVm,
	...
	if (search != VM.workingVmSet.end()) {      // 表示该虚拟机在前几天的服务器里
		...
		if (requestVm.nodeStatus) {   // 双节点
			...
			server.updatVmTarOrder(requestVm.needCPU/2, requestVm.needRAM/2, requestVm.needCPU/2, requestVm.needRAM/2, serID, false);
		}
		else {    // 单节点
