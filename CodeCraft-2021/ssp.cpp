...
*/
	...
	for (const auto &xSer : server.info) {
		...
		tie(ocpRes, vmPath) = dp(curSet.size(), totalCPU / 2, totalRAM / 2, totalCPU / 2, totalRAM / 2, \
			curSet, VM);
...
tuple<int, stack<int>> dp(int N, int aIdleCPU, int aIdleRAM, int bIdleCPU, int bIdleRAM, \
	vector<pair<string, string>> &curSet, const cVM &VM) 
{
/*
* Note: 目前没有记忆，只是单纯递归，规模如果需要很大可能自定义类型哈希表
*/
	stack<int> resPath; // 各个vm的处理结果 ref: knapSack函数的注释
	...
	stack<int> path, path0, path1, path2; // path0不装入路径 path装入路径 path1/path2同y1 y2解释

	/*递归*/
	tie(x, path0) = dp(N - 1, aIdleCPU, aIdleRAM, bIdleCPU, bIdleRAM);
	path0.push(0);

	if (vmIsDouble) {
		tie(y, path) = dp(N - 1, aIdleCPU - needCPU / 2, aIdleRAM - needRAM / 2, bIdleCPU - needCPU / 2, \
			bIdleRAM - needRAM / 2) + needCPU + needRAM;
		path.push(1);
	}

	else {
		// single node a
		tie(y1, path1) = dp(N - 1, aIdleCPU - needCPU, aIdleRAM - needRAM, bIdleCPU, bIdleRAM) + needCPU + needRAM;
		// single node b
		tie(y2, path2) = dp(N - 1, aIdleCPU, aIdleRAM, bIdleCPU - needCPU, bIdleRAM - needRAM) + needCPU + needRAM;
		if (y1 > y2) {
			path1.push(2);
			path = path1;
			y = y1;
